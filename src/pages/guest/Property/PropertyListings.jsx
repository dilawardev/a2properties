...
const PropertyListings = () => {
  ...
  const normalizeListingResponse = useCallback((res) => {
    ...
    const summarizeRange = (min, max) => {
      if (min != null && max != null) return min === max ? min : `${min}-${max}`;
      return min ?? max ?? null;
    };

    const deriveNewStats = (raw) => {
      const newParam =
        raw?.newParam ??
        raw?.newParameter ??
        raw?.new_parameter ??
      ...
      const plans = [
        ...(Array.isArray(newParam?.floorPlan) ? newParam.floorPlan : []),
        ...(Array.isArray(newParam?.floorplan) ? newParam.floorplan : []),
        ...(Array.isArray(newParam?.floor_plan) ? newParam.floor_plan : []),
        ...(Array.isArray(newParam?.style) ? newParam.style : []),
      ];
      ...
      const pushBed = (val) => {
        ...
        if (num != null) bedValues.push(num);
        else if (val) bedText.push(String(val).trim());
      };

      pushBed(newParam?.bedroomMin ?? newParam?.bedRoomMin);
      pushBed(newParam?.bedroomMax ?? newParam?.bedRoomMax);

      plans.forEach((plan) => {
        pushBed(plan?.bedroom ?? plan?.bedRoom ?? plan?.beds ?? plan?.bedrooms);
        const parsed = parseBedsFromName(plan?.name);
        if (parsed) pushBed(parsed);
      });

      const sizeValues = [];
      const pushSize = (val) => {
        const num = toPositiveNumber(val);
        if (num != null) sizeValues.push(num);
      };

      pushSize(newParam?.minSize);
      pushSize(newParam?.maxSize);

      plans.forEach((plan) => {
        pushSize(plan?.area ?? plan?.size ?? plan?.sqft ?? plan?.squareFeet);
      });

      const beds =
        summarizeRange(
          bedValues.length ? Math.min(...bedValues) : null,
          bedValues.length ? Math.max(...bedValues) : null,
        ) ?? bedText.find(Boolean) ?? null;

      const size =
        summarizeRange(
          sizeValues.length ? Math.min(...sizeValues) : null,
          sizeValues.length ? Math.max(...sizeValues) : null,
        ) ?? null;
    ...
    const toListing = (entry) => {
      ...
      const { beds: derivedBeds, size: derivedSize } = deriveNewStats(r);

      const normalizedBeds =
        toPositiveNumber(p.beds) ??
        toPositiveNumber(r.bedRooms ?? r.bedrooms) ??
        derivedBeds ??
        null;

      const normalizedSize =
        toPositiveNumber(p.size) ??
        toPositiveNumber(r.size ?? r.plotSize) ??
        derivedSize ??
        null;
